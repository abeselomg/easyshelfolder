...
class BookList extends StatefulWidget with NavigationStates {
  @override
...
class _BookList extends State<BookList> {
  ...
  Future _booksData () async {
    ...
    if(selectedCatsList.isEmpty){
      url = urlSt+ '/api/v1/allbooks';
    }else {
      List<int> selectedList = [];
      selectedCatsList.forEach((element) {
        selectedList.add(element.id);
      });
      url = urlSt+ '/api/v1/bookfliterbycatagories?catagory_id='+selectedList.join(",");
    }
